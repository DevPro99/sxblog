            ...
            for (var c = 0; c < g.length; ++c) {
              ...
              for (var b = r, k = i; b < t.length; k += t[b].length, ++b) {
                ...
                if (!(w instanceof s)) {
                  ...
                  if (_) {
                    d && (p = _[1].length);
                    var A = _.index + p, _ = _[0].slice(p), j = A + _.length, N = w.slice(0, A), C = w.slice(j),
                        E = [b, P];
...
Prism.languages.elixir = {
  ...
  e.inside = {
    interpolation: {
      pattern: /#\{[^}]+\}/,
      inside: {delimiter: {pattern: /^#\{|\}$/, alias: "punctuation"}, rest: Prism.util.clone(Prism.languages.elixir)}
    }
  }
});
...
Prism.languages.scss = Prism.languages.extend("css", {
  ...
  selector: {
    pattern: /(?=\S)[^@;\{\}\(\)]?([^@;\{\}\(\)]|&|#\{\$[-_\w]+\})+(?=\s*\{(\}|\s|[^\}]+(:|\{)[^\}]+))/m,
    inside: {parent: {pattern: /&/, alias: "important"}, placeholder: /%[-_\w]+/, variable: /\$[-_\w]+|#\{\$[-_\w]+\}/}
  }
      ...
      pattern: /(^|\r|\n)\S[\s\S]*?(?=$|\r?\n\r?\n|\r\r)/, lookbehind: !0, inside: {
        ...
        inline: {
          ...
          inside: {
            ...
            cite: {pattern: RegExp("(^\\?\\?(?:" + i + ")?).+?(?=\\?\\?)"), lookbehind: !0, alias: "string"},
